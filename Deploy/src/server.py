...
class Client:
    ...
    def main(self):
        try:
            while True:
                ...
                # Image stream
                    ...
                    try:
                        ...
                        self.sock.send(
                            int.to_bytes(self.server.commandDict["failure"], 2, "big")
                        )
                        continue

                    self.sock.send(
                        int.to_bytes(self.server.commandDict["success"], 2, "big")
                    )

                elif command == self.server.commandDict["overlay"]:
                    try:
                        self.server.camera.overlay = int.from_bytes(
                            self.sock.recv(2), "big"
                        )
                    except Exception as e:
                        print(f"[ERR] {str(type(e))}: {str(e)}")
                        self.sock.send(
                    ...
                    try:
                        self.server.camera.exposure = (
                            int.from_bytes(buf[:2], "big"),
                            int.from_bytes(buf[2:], "big"),
                        )
                    except Exception as e:
                        print(f"[ERR] {str(type(e))}: {str(e)}")
                    ...
                    try:
                        self.server.camera.focus = int.from_bytes(
                            self.sock.recv(2), "big"
                        )
                    except Exception as e:
                        print(f"[ERR] {str(type(e))}: {str(e)}")
            ...
            print(
                f"[ERR] A fatal error occurred on client at address {self.addr}, reduce number of simultaneous connections."
            )

        except Client.InvalidCommandError:  # Undefined command received
            print(
                f"[Err] Invalid command received in client at address {self.addr}, could be due to bad packets. {command}"
            )
    ...
    def main(self, camera):
        ...
        while True:
            ...
            if len(self.clients) < self.max_connections:
                ...
                threading.Thread(
                    target=self.clients[address].main, name=str(address[1]), daemon=True
                ).start()
            else:
                print(
                    f'[WARN] Denied client {address} from connecting.\
